...
# ponytail: L2 거리 임계값. 이보다 먼(덜 관련된) 청크는 버려 노이즈를 줄인다.
# 실데이터 점수 분포로 정한 보수값(명백한 무관 질문은 컷됨). 답변 품질 보며 조정 가능.
CHROMA_MAX_DISTANCE = 1.2
...
def _get_vectorstore() -> Chroma:
    """'paiM' 컬렉션을 LangChain Chroma 벡터스토어로 감싸 반환(싱글톤).
    적재측(db/chroma.py)과 동일한 OpenAI 임베딩(EMBED_MODEL)을 써야 벡터가 맞는다."""
    global _vectorstore
