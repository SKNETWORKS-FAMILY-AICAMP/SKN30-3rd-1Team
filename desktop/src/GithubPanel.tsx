...
function getGithubMemorySourceType(item: ProjectMemoryItem) {
  return item.source_info?.type ?? "";
}

function getGithubMemorySourceLabel(item: ProjectMemoryItem) {
  const sourceType = getGithubMemorySourceType(item);
...
function getGithubLinkedMemoryItems(memoryItems: ProjectMemoryItem[], repository?: GitRepositoryInfo) {
  return memoryItems.filter((item) => isGithubMemoryLinked(item, repository));
}
...
function getGithubTimelineTabLabel(type: GitHubEventType) {
...
export function GithubPanel({
  ...
  const linkedMemoryItems = getGithubLinkedMemoryItems(memoryItems, repository);
  const memoryDisplayItems = getGithubMemoryDisplayItems(memoryItems, repository);
  const connectedMemoryTotal = Math.max(events.length, linkedMemoryItems.length);
  const visibleTimelineEvents = events;
      ...
      {repository ? (
        <div className="overview-github-connected-shell">
          ...
          <div className="overview-github-summary-strip">
            ...
            <div className="overview-github-spark" aria-label={t("커밋 흐름")}>
          ...
          <div className="overview-github-connected-grid">
            <section className="overview-github-sync-column">
              <div className="overview-github-memory-heading">
                ...
                <span>
                  {t("{linked} / {total} 연결됨", {
                    linked: linkedMemoryItems.length,
                    total: connectedMemoryTotal,
                  })}
                </span>
              </div>
              ...
              {connectedMemoryTotal > linkedMemoryItems.length ? (
                <div className="overview-github-memory-suggestion">
                  <p>{t("연결 제안")}</p>
                  <small>
                    {t("아직 메모리로 연결되지 않은 GitHub 항목이 있습니다.")}
                  </small>
                </div>
              ) : null}
            </section>
            ...
            <section className="overview-github-timeline-section">
              <div className="overview-github-log-tabs" role="tablist" aria-label={t("최근 활동")}>
                {(["commit", "pull_request", "issue"] as GitHubEventType[]).map((type) => (
                  <button
                    className="overview-github-log-tab"
                    data-active={type === "commit"}
                    key={type}
                    role="tab"
                    type="button"
                  >
                    <SvgIcon svg={getGithubEventIconSvg(type)} />
                    <span>{t(getGithubTimelineTabLabel(type))}</span>
                    <strong>{eventCounts[type]}</strong>
                  </button>
                ))}
              </div>
              {visibleTimelineEvents.length > 0 ? (
                <div className="overview-timeline-list">
                  {visibleTimelineEvents.map((event, index) => {
                    ...
                    const previousAge =
                      index > 0
                        ? formatRelativeAge(visibleTimelineEvents[index - 1].createdAt, language)
                        : null;
                      ...
                      <div className="overview-timeline-entry" key={event.id}>
                        ...
                        <div className="overview-timeline-row">
                          ...
                          >
                            ...
                            {index < visibleTimelineEvents.length - 1 ? <span /> : null}
                          </div>
